...
function chkpkg() {
	...
	do
		...
		if [ "" = "$PKG_OK" ]; then
		  ...
		  apt-get --yes install $package --fix-missing 1> /dev/null
		fi
	done
}

# Helper function for sourced or run directly
# From: https://stackoverflow.com/questions/2683279/how-to-detect-if-a-script-is-being-sourced/23009039#23009039
# Sample usage: is_sourced && sourced=1 || sourced=0
#
function is_sourced() {
...
useradd -m -s /bin/bash -g $GID $USER_NAME
printf "${green}Enter a password for <$USER_NAME>...\n"
passwd $USER_NAME
usermod -aG docker $USER_NAME      # Add a docker group so we won't have to sudo
...
mkdir -p $MOUNT_POINT/$GROUP_NAME
chown -R $USER_NAME:$GROUP_NAME /$MOUNT_POINT/$GROUP_NAME
chmod -R g+w $MOUNT_POINT/$GROUP_NAME
